...
export class JobFormComponent implements OnInit, OnDestroy {
  ...
  onSubmit(): void {
    ...
    this.jobResult = null; // Clear any previous general jobResult
    this.jobPredictionResult = null; // Clear any previous prediction result
    this.showModal = false;
    this.isSubmitting = true; // Start submitting state

    // Recalculate risk level one last time before making a decision
    this.calculateRiskLevel();

    // --- Logic for 'Low Risk' override ---
    if (this.riskLevel === 'Low Risk') {
      console.log('Low Risk detected by form, displaying immediate legitimate result.');
      this.jobResult = {
        isFake: false, // Force to legitimate
        message: 'Job Appears Legitimate',
        details: ['Based on our initial assessment, this job post is considered low risk.']
      };
      this.jobPredictionResult = {
        is_fraudulent: false,
        fraud_probability: 0.05, // Example low probability
        confidence: 0.95,
        legitimate_probability: 0.95,
        risk_level: 'TRÈS BAS', // Ensure consistency with a very low risk
        extracted_features: ['Assessment: Form-based risk assessment was Low Risk.']
      };
      this.showModal = true;
      this.isSubmitting = false;
      console.log('✅ Job Appears Legitimate (Low Risk Override)');
      return; // Stop execution here, do not proceed to API call
    }
    // --- End 'Low Risk' override logic ---

    // If not 'Low Risk', proceed with standard form validation and API call
    ...
    console.log('Form is valid and risk level is not Low Risk. Submitting job data to API...');
    ...
    this.jobService.predictJobFraud(jobData).subscribe({
      next: (fraudPredictionResult: FraudPredictionResult) => {
        ...
        this.jobPredictionResult = {
          ...
          extracted_features: fraudPredictionResult.extracted_features, // Still store full features internally
        };

        // --- MODIFIED LOGIC START: Simplify jobResult.details for the modal ---
        let detailsMessage: string;
        if (fraudPredictionResult.is_fraudulent) {
          detailsMessage = 'This job post has been identified as potentially fraudulent by our system.';
        } else {
          detailsMessage = 'This job post appears to be legitimate based on our analysis.';
        }

        this.jobResult = {
          isFake: fraudPredictionResult.is_fraudulent, // Based on API's determination
          message: fraudPredictionResult.is_fraudulent ? 'Fraudulent Job Detected' : 'Job Appears Legitimate',
          details: [detailsMessage] // Simplified detail message
        };
        // --- MODIFIED LOGIC END ---
  ...
  onScraperSubmit(): void {
    ...
    of({
      ...
      .subscribe(result => {
        ...
        // When using the scraper, we don't have a FraudPredictionResult directly,
        // so we derive a simple one for display consistency if needed by the modal's current structure.
        // If your modal relies solely on jobPredictionResult, you might need to adjust this.
        // For simplicity, let's just make sure jobPredictionResult is set if jobResult is being used for display.
        if (this.jobResult) {
          this.jobPredictionResult = {
            is_fraudulent: this.jobResult.isFake,
            fraud_probability: this.jobResult.isFake ? 0.8 : 0.1, // Example values
            confidence: 0.9,
            legitimate_probability: this.jobResult.isFake ? 0.2 : 0.9,
            risk_level: this.jobResult.isFake ? 'TRÈS ÉLEVÉ' : 'TRÈS BAS', // Map to risk level
            extracted_features: this.jobResult.details // Pass the details from the scraper result
          };
          // Also, update the `riskLevel` property for consistency with the form's real-time indicator.
          // This is important because the modal's display also depends on `riskLevel`.
          if (this.jobPredictionResult.risk_level === 'TRÈS BAS') {
            this.riskLevel = 'Low Risk';
          } else if (this.jobPredictionResult.risk_level === 'MOYEN') {
            this.riskLevel = 'Medium Risk';
          } else if (this.jobPredictionResult.risk_level === 'ÉLEVÉ') {
            this.riskLevel = 'High Risk';
          } else if (this.jobPredictionResult.risk_level === 'TRÈS ÉLEVÉ') {
            this.riskLevel = 'Very High Risk';
          }
        }
        this.showModal = true;
  ...
  closeModal(): void {
    this.showModal = false;
    this.jobResult = null;
    this.jobPredictionResult = null; // Also clear jobPredictionResult
  }
